...
def create_app():
    ...
    def index():
        ...
        return render_template('dashboard.html', total=total or 0, open_count=open_count or 0, progress_count=progress_count or 0, closed_count=closed_count or 0, recent=recent)
    ...
    def register():
        ...
        if form.validate_on_submit():
            ...
            user = User(name=form.name.data.strip(), email=form.email.data.lower(), is_admin=form.is_admin.data)
    ...
    def ticket_new():
        form = TicketForm()
        form.assigned_to.choices = [(0, '-- Nessuno --')] + [(u.id, f"{u.name} ({u.email})") for u in db.session.query(User).order_by(User.name).all()]
        if form.validate_on_submit():
            filename = None
            ...
            t = Ticket(title=form.title.data.strip(), description=form.description.data.strip(), status=TicketStatus.OPEN, priority=form.priority.data, created_by_id=current_user.id, assigned_to_id=form.assigned_to.data if form.assigned_to.data else None, attachment=filename)
            db.session.add(t)
            db.session.flush()
            action = TicketAction(ticket_id=t.id, user_id=current_user.id, action='CREAZIONE', notes='Ticket creato')
    ...
    def ticket_detail(ticket_id):
        ...
        if form.validate_on_submit():
            ...
            act = TicketAction(ticket_id=t.id, user_id=current_user.id, action=("; ".join(changes) if changes else "Aggiornamento"), notes=notes)
            ...
            return redirect(url_for('ticket_detail', ticket_id=t.id))
        actions = (db.session.query(TicketAction).filter(TicketAction.ticket_id == ticket_id).order_by(TicketAction.created_at.desc()).all())
